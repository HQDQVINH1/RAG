"""
Module RAG Pipeline cho Buổi 07.

Bước 04: Data Loader & Validator cho JSON Chunks.
Bước 05: Gemini Embedding & ChromaDB Persistent Indexing.
Bước 06: Retrieval, Grounding & Evidence Citation.
- Đường dẫn động dựa trên Path(__file__).resolve().
- Đọc cấu hình từ .env bằng python-dotenv.
- Tạo & Validate Gemini Embeddings.
- Lưu trữ & Query ChromaDB Persistent với Cosine metric.
- Lọc Confidence Gate (RAG_MAX_DISTANCE).
- Grounding Generation & Citation Mapping chuẩn xác.
- Hỗ trợ CLI commands: validate, status, index, query.
"""
...
# Cấu hình đường dẫn dựa trên vị trí file rag.py
...
def load_config() -> Dict[str, Any]:
    """
    Đọc và kiểm tra cấu hình từ file .env.
    """
    ...
    for attempt in range(5):
        try:
            ...
            if ("429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg) and attempt < 4:
                ...
                print(f"[RATE LIMIT] Đạt giới hạn API request. Tự động chờ {wait_time}s (lần thử {attempt + 1}/4)...")
...
def generate_embeddings(
    ...
    Validate toàn bộ vector trước khi trả về.
    """
    embeddings = []
    for idx, c in enumerate(chunks, start=1):
        ...
        time.sleep(0.6)  # Pacing rate limit
...
def query_rag(
    ...
    """
    Quy trình RAG Query hoàn chỉnh (Bước 06):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="RAG Pipeline - Data Loader, Validator, Indexer & Query Engine")
    ...
    if args.command == "validate":
        try:
            ...
            if chunks:
                ...
                print(f"\nHIỂN THỊ {sample_count} METADATA MẪU:")
                for i, c in enumerate(chunks[:sample_count], start=1):
                    ...
                    print(f"Mẫu #{i}: {json.dumps(meta_sample, ensure_ascii=False)}")
        except Exception as e:
        ...
        try:
            ...
            print("==================================================")
            print("TRẠNG THÁI HỆ THỐNG RAG (READ-ONLY)")
            print("==================================================")
        ...
        try:
            ...
            print("==================================================")
            print("KẾT QUẢ INDEXING RAG CHUNKS")
            print("==================================================")
        ...
        try:
            ...
            print(res["answer"])
            print("--------------------------------------------------")
            print(f"DANH SÁCH BẰNG CHỨNG (EVIDENCE) [{len(res['evidence'])}]:")
            for ev in res["evidence"]:
                status_str = "HỢP LỆ" if ev["accepted"] else "BỎ QUA (VƯỢT THRESHOLD)"
                page_str = format_page_str(ev["page_start"], ev["page_end"])
                text_preview = ev["text"][:100].replace("\n", " ") + ("..." if len(ev["text"]) > 100 else "")
                print(f"  - [{ev['evidence_id']}] ({status_str}) | Distance: {ev['distance']}")
                print(f"    Source: {ev['source']} | {page_str} | Chunk ID: {ev['chunk_id']}")
                print(f"    Preview: \"{text_preview}\"")
            
            if res["warnings"]:
                print("--------------------------------------------------")
                print(f"CẢNH BÁO (WARNINGS) [{len(res['warnings'])}]:")
